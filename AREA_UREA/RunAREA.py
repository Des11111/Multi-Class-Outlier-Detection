...
from make_mpu_data import make_mpu_train_set
    ...
    for epoch in range(epoch_num):
        for batch_data in train_dataloader:
            ...
            if labeled_num > 0:
                ...
                class_num_per_instance = labeled_batchY.sum(dim=0)[y_labeled_batch].float() # number of examples in each class for each instance, n
                pred_labeled = model(labeled_batchX)
                labeled_margin = (pred_labeled*labeled_batchY).sum(dim=1)
                loss_1_temp = loss_fn(labeled_margin) + loss_fn(-pred_labeled[:,-1])
                loss_1_average = (K/(K-1))*((loss_1_temp * class_prior_per_instance)/class_num_per_instance).sum()
...
if __name__ == '__main__':
    ...
    test_size = 0.2
    unlabeled_ratio = 0.5
    loss_name = "hinge" # hinge, sigmoid, ramp
    epoch_num = 200
    ...
    model = LinearClassifier(X_labeled_train.shape[1], Y_labeled_train.shape[1])   #LinearClassifier and NNClassifier
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4) #1e-3,1e-4 for usps
